#!/usr/bin/env bash
# Runs the Paxos example end to end: one name server plus three nodes (n1,
...
go build -o "$bin/nameserver" ./examples/paxos/nameserver
go build -o "$bin/node" ./examples/paxos
