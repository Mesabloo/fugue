...
| Diagnostic/error-model shape | **Per-pass error types, unified by a common rendering interface** — not one shared diagnostic sum type. Warning suppression (`-W`/`-Wno-<name>`) = filtering before rendering: passes raise everything, `PipelineResult.warnings` keeps unfiltered record, `PipelineResult.reportedWarnings` applies filter. Progress line's `⚠ Built <Module>` follows *reported* warnings, not raised ones — suppressed warning silenced everywhere, dingbat included (`ModuleOutcome.built`, both `.built` sites filter). Mechanism exists in `Common/Errors.lean` (§4) — read before designing new. Fine to refactor error style or emission mechanism later if it doesn't hold up. |
...
│   ├── ComputableTLAPlus/ ComputablePlusCal/    output of a separate pass *after* §5.3, not of the checker itself (§5.3)
│   ├── GuardedPlusCal/                          guards floated to block-start (§5.4)
...
- **Variable well-scopedness.** Every variable reference resolves to a declared name of the
  ...
  type-checks) and stays load-bearing. `WellFormedness/WellScoped/GuardedPlusCal.lean`
  ...
  (alongside `WellFormedness/WellScoped/GuardedPlusCal.lean`) are the frontend half of the general renaming/hygiene
...
finite runtime representation. Phased separately (§7). The ported well-scopedness check
(`WellFormedness/WellScoped/TypedPlusCal.lean`, §5.2a) already ran on this pass's *input*,
`TypedTLAPlus`/`TypedPlusCal` — not re-run on `ComputableTLAPlus`/`ComputablePlusCal`.
...
- **`<:` is a genuine partial order, not just a preorder** — structural rules
  ...
  exactly this partial `lub`. Whether to make it a real join instead: §9.18.
...
`WellFormedness/WellScoped/GuardedPlusCal.lean` (§5.2a) supplies the well-scopedness
...
- **Equality/ordering: one dictionary, passed explicitly.** Go's `==`/`comparable` can't be
  ...
  they order — true of `Set[T]`'s tagged-struct representation too (below, §9.15).
  `persistent/treemap` (`New(cmp func(a, b K) int)`) is the precedent.
- **Booleans.** `/\`/`\/` → Go's short-circuiting `&&`/`||` (non-action, non-temporal TLA+
  ...
  call rather than an inlined Go loop, since `len`/index don't compile directly against
  `Set` — not literally a slice (§9.15).
...
declared or `fifos`-declared alike — the source-level `Set(τ)` encoding the thesis suggests
for `channels` (§3.1.1) never reaches this state space, so `Guarded2Network`'s inbox gives
every mailbox FIFO order regardless of declaration kind (§9.38).
...
`StrongRefinement`s. Alternative weighed and rejected: existential `pref` + a frame lemma
on the source block.
...
7. **Well-formedness checking — done** (§5.2a): well-labelledness, variable
   ...
   well-scopedness is load-bearing here. `WellFormedness/WellScoped/{TypedPlusCal,GuardedPlusCal}.lean`
   ported here (proof-support at phases 9–10); `CorePlusCal.WellScoped` authored fresh.
